...
void rtc_task(void *param)
{
	...
	const char *msg_rtc_daytime = "Enter day time(AM: 0, PM: 1):";
			...
			switch(curr_state)
			{
				...
				case sRtcTimeConfig:{
					...
					case HH_CONFIG:
						...
						break;
					case MM_CONFIG:
						minute = getnumber(cmd->payload, cmd->len);
						time.Minutes = minute;
						rtc_state = SS_CONFIG;
						xQueueSend(q_print, &msg_rtc_ss, portMAX_DELAY);
						break;
					case SS_CONFIG:
						second = getnumber(cmd->payload, cmd->len);
						time.Seconds = second;
						rtc_state = DT_CONFIG;
						xQueueSend(q_print, &msg_rtc_daytime, portMAX_DELAY);
						break;
					case DT_CONFIG:{
						if((cmd->payload[0] - 48) == 0)
						{
...
//							rtc_configure_time(&time);
				...
				case sRtcDateConfig:{
					...
					{
						case DATE_CONFIG:
							dd = getnumber(cmd->payload, cmd->len);
							date.Date = dd;
